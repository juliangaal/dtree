...
GraphGenerator::GraphGenerator(const Node &root, const vector<string> labels, 
                               const GraphType type, const string filepath) :
  labels(labels), graph_type(type) {
...
void GraphGenerator::generate(shared_ptr<Node> root, const string& filepath) {
  ...
  myfile << "digraph {\n";
  parseTree(root, myfile);
  myfile << "}";
  myfile.close();
}

void GraphGenerator::parseTree(shared_ptr<Node> node, ofstream &file) {
  switch(graph_type) {
    case VERBOSE:
      parseVerboseTree(node, file);
      break;
    case SIMPLE:
    default:
      parseSimpleTree(node, file);
      break;
  }
}

void GraphGenerator::parseSimpleTree(shared_ptr<Node> node, ofstream &file) {
  ...
  parseTree(node->false_branch_, file);
}

void GraphGenerator::parseVerboseTree(shared_ptr<Node> node, ofstream &file) {
  static const string spacing = "   ";
  static const string connector = " -> ";
  if (node == nullptr)
    return;
